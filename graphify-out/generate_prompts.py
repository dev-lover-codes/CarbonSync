...
files = [line.strip() for line in Path('graphify-out/.graphify_uncached.txt').read_text(encoding='utf-8').splitlines() if line.strip()]
chunk_size = 25
num_chunks = math.ceil(len(files) / chunk_size)

for i in range(num_chunks):
    chunk_files = files[i*chunk_size : (i+1)*chunk_size]
    chunk_path = str(Path('.').resolve() / 'graphify-out' / f'.graphify_chunk_{i+1:02d}.json')
...
Files (chunk {i+1} of {num_chunks}):
{chr(10).join(chunk_files)}
...
{chunk_path}"""
    Path(f'graphify-out/.prompt_chunk_{i+1}.txt').write_text(prompt, encoding='utf-8')
print(f'Wrote {num_chunks} prompts')
