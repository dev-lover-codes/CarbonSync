...
export const createUserProfile = async (uid, data) => {
  ...
  try {
    ...
    console.warn("Firestore createUserProfile failed, falling back to localStorage:", error);
    const users = JSON.parse(localStorage.getItem("mock_users") || "{}");
    users[uid] = {
      ...data,
      totalSaved: data.totalSaved !== undefined ? data.totalSaved : 0,
...
export const updateUserProfile = async (uid, data) => {
  ...
  try {
    ...
    console.warn("Firestore updateUserProfile failed, falling back to localStorage:", error);
    const users = JSON.parse(localStorage.getItem("mock_users") || "{}");
    if (users[uid]) {
      users[uid] = { ...users[uid], ...data };
      localStorage.setItem("mock_users", JSON.stringify(users));
...
export const logActivity = async (uid, activityData) => {
  ...
  try {
    ...
    console.warn("Firestore logActivity failed, falling back to localStorage:", error);
    const key = `mock_activities_${uid}`;
    const activities = JSON.parse(localStorage.getItem(key) || "[]");
    const newAct = {
      id: Math.random().toString(36).substr(2, 9),
    ...
    if (users[uid]) {
      const change = activityData.co2 < 0 ? Math.abs(activityData.co2) : 0;
      users[uid].totalSaved = (users[uid].totalSaved || 0) + change + 1.0;
      localStorage.setItem("mock_users", JSON.stringify(users));
    }
  }
};
...
export const getActivities = async (uid, limitCount = 10) => {
  ...
  try {
    ...
    console.warn("Firestore getActivities failed, falling back to localStorage:", error);
    const activities = JSON.parse(localStorage.getItem(`mock_activities_${uid}`) || "[]");
    return activities.slice(0, limitCount);
  }
};
...
export const saveGoal = async (uid, goalData) => {
  ...
  try {
    ...
    console.warn("Firestore saveGoal failed, falling back to localStorage:", error);
    const key = `mock_goals_${uid}`;
    const goals = JSON.parse(localStorage.getItem(key) || "[]");
    const newGoal = {
      id: Math.random().toString(36).substr(2, 9),
...
export const getGoals = async (uid) => {
  ...
  try {
    ...
    console.warn("Firestore getGoals failed, falling back to localStorage:", error);
    return JSON.parse(localStorage.getItem(`mock_goals_${uid}`) || "[]");
  }
};
...
export const updateGoal = async (uid, goalId, data) => {
  ...
  try {
    ...
    console.warn("Firestore updateGoal failed, falling back to localStorage:", error);
    const key = `mock_goals_${uid}`;
    const goals = JSON.parse(localStorage.getItem(key) || "[]");
    const idx = goals.findIndex(g => g.id === goalId);
    if (idx !== -1) {
...
export const getLeaderboard = async () => {
  ...
  try {
    ...
    console.warn("Firestore getLeaderboard failed, falling back to localStorage:", error);
    const users = JSON.parse(localStorage.getItem("mock_users") || "{}");
    const list = Object.keys(users).map(uid => ({ id: uid, ...users[uid] }));
    if (list.length < 5) {
      const mockLeaders = [
...
export const saveInsight = async (uid, insightData) => {
  ...
  try {
    ...
    console.warn("Firestore saveInsight failed, falling back to localStorage:", error);
    const key = `mock_insights_${uid}`;
    const insights = JSON.parse(localStorage.getItem(key) || "[]");
    const newInsight = {
      id: Math.random().toString(36).substr(2, 9),
...
export const getInsights = async (uid) => {
  ...
  try {
    ...
    console.warn("Firestore getInsights failed, falling back to localStorage:", error);
    const key = `mock_insights_${uid}`;
    const insights = JSON.parse(localStorage.getItem(key) || "[]");
    if (insights.length === 0) {
      const defaultInsight = {
